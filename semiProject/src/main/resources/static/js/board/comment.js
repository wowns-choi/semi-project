 ...
 * @param {*} btn 
 */
const showInsertComment = (parentComment, btn) => {
  ...
  insertBtn.setAttribute("onclick", "insertChildComment("+parentComment+", this)");
...
/** 답글 (자식 댓글) 등록
 * @param {*} parentComment : 부모 댓글 번호
 * @param {*} btn  :  클릭된 등록 버튼
 */
const insertChildComment = (parentComment, btn) => {
  ...
  const data = {
    ...
    "boardNo"        : boardNo,
    "memberNo"       : loginMemberNo,  // 또는 Session 회원 번호 이용도 가능
    "parentComment" : parentComment // 부모 댓글 번호
  };
